...
Verifying a webhook actually came from who it claims to be usually means checking a signature the sender computes over the payload with a shared secret (HMAC), sent in a header — since anyone can guess a webhook URL and POST arbitrary JSON at it, and a receiver that trusts the body blindly is trusting the internet. An [[API Gateway]] sitting in front of the receiving endpoint is a natural place to enforce that check once, rather than in every handler.

Webhooks are also a poor fit for anything that needs a response mid-transaction — they're fire-and-forget by design, which is why bidirectional real-time needs look instead to [[WebRTC]] or a persistent socket, not a webhook.
