...
Concretely, native elements bring behaviour that is otherwise expensive to rebuild: focusability, keyboard activation, form participation, correct announcement, and platform conventions that vary by operating system. Reimplementing that on a generic element is a lot of code that will be subtly wrong, which is why [[ARIA]]'s first rule is to prefer the native element.
