...
A [[Regular Expression]] is exactly a finite state machine in another notation — a formal regular expression compiles to an FSM (as engines like RE2 and Rust's `regex` still do, though PCRE-family engines use backtracking instead), which is the concrete reason the class of language it can match is bounded (it can count "any number of a's" but not "an equal number of a's and b's," since that requires unbounded memory a finite state machine doesn't have).
