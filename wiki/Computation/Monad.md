...
The laws buy a license to refactor: regrouping cannot change meaning, so a sub-chain can be extracted into a named helper without argument about semantics. They are also testable as properties, one of the few places a [[Unit Test]] checks an algebraic claim rather than an example.
