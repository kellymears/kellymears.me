...
---
**Trie** (pronounced "try," from re**trie**val) stores a set of strings as a tree where each edge is labeled with a character and each node represents the prefix spelled out by the path from the root to it — so "car" and "cart" share every node up through "car," diverging only at the fourth character. A lookup, insert, or prefix check walks the tree one character at a time and costs time proportional to the string's length, not to how many strings are stored — a property no comparison-based structure or [[Hash Table]] shares, since a hash table's lookup is fast but tells you nothing about prefixes without a separate structure.
