...
---
**Least recently used (LRU) cache** is a bounded store that, when it needs room, discards the entry that has gone untouched the longest. It is the default eviction policy in page caches, database buffer pools, and memoization, because it is cheap and assumes nothing about the workload.
