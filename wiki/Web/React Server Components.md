...
---
**React Server Components** are components that run exclusively on the server. They can read a database, use the filesystem, and hold secrets, and their code is never sent to the browser — the client receives a serialized description of their output instead of the logic that produced it.
...
The rules that follow are strict by necessity. Anything crossing the boundary must be serializable, so functions and class instances cannot be passed to a client component. Hooks and event handlers require a client component, marked explicitly. And a client component's imports are all client code — including anything it imports transitively, which is how a single innocuous import can put a large library into the visitor's bundle. See [[Module Graph]] and [[Code Splitting]].
