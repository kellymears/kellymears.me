...
Making it explicit has three effects. It determines what ships: everything on the client side of the line is downloaded, parsed, and executed by every visitor. It determines what is reachable: secrets, direct database access, and the filesystem exist on one side only. And it determines what must be serializable, since only data crosses.
...
The commonest way it goes wrong is accidental: an import added for convenience drags a module across the line, and a large dependency lands in every visitor's bundle. Lint rules restricting which modules may be imported from which directories are the standard defense.
