...
The trade is freshness and combinatorics. Content that changes requires a rebuild, and anything genuinely per-request — personalization, authenticated views — cannot be baked in. Sites with very large page counts pay build time proportional to that count.
