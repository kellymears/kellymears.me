...
The visible behavior while a font is pending is controlled by `font-display`. The choices trade a flash of unstyled text against a flash of *invisible* text; the latter is worse for perceived performance and for [[Core Web Vitals]], since a layout shift lands when the real face arrives.
