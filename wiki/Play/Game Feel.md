...
The vocabulary is small and largely portable. **Input buffering** accepts a press made slightly before its window opens and fires it when the window arrives. **Coyote time** leaves a jump available for a few frames after the character has left the ledge. **Hitstop** freezes both parties on impact, communicating force through the absence of motion. **Screen shake** and impact flashes supply magnitude. **Animation timing** — startup, active and recovery frames, and the windows in which an animation may be canceled — decides whether an input feels obeyed or ignored. A [[Rhythm Game]] makes the same timing budget explicit, scoring a press against a window instead of merely feeling it, but the underlying enemy — input latency — is identical.

The useful claim is that "the controls feel wrong" is almost always a tuning problem rather than a physics problem. The instinct is to change gravity or acceleration constants; the fault is more often thirty milliseconds of uncancelable startup, an input sampled once per rendered frame instead of once per fixed step, a deadzone that swallows small stick movements, or a linear ramp where the eye expects an [[Easing]] curve. The rules that govern interface [[Motion Design]] apply here, because the player is reading motion. Motion also raises the bar a humanlike character has to clear: a face that holds up as a still image can fall into the [[Uncanny Valley]] once it moves and its timing and tracking cues go slightly wrong, which is part of why deliberate stylization is the safer setting for a body the player controls.
