...
**The payoff is a solution space larger than the design document.** A locked door yields to the key, the lockpick, the vent above it, the guard who can be persuaded to open it, or the wall beside it if the wall is breakable — and crucially, to combinations nobody enumerated. This is emergence in the ordinary sense: consistent local rules producing global behavior that was not specified, the same phenomenon a [[Cellular Automaton]] demonstrates with far less machinery. The designer's job shifts from authoring paths to authoring rules and then verifying that the space they open is navigable.
