...
---
**Rollback** reverts a deployed system to a previous version after a release turns out to be bad. It is the release-time counterpart to `git revert` in source control: instead of undoing the change in history, you redeploy an artifact that already worked. The whole point is speed — a rollback exists to stop the bleeding *now*, not to diagnose or fix the underlying bug, which can happen afterward at normal pace.
