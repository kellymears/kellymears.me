...
The diagnostic signature is consistent and worth memorizing: **a timeout, never an assertion failure**, in a file unrelated to whatever changed, that passes in isolation. A test asserting something wrong fails the same way every time; a starved test fails on the timing and passes alone.
