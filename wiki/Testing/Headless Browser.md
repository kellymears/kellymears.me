...
The essential distinction is between a headless *browser* and a simulated DOM. A simulated environment is fast and cannot evaluate layout, cascade, or focus, so tests that appear to check visual behavior there are checking a model of it. When the question is visual or interactive, only a real engine answers it.

Automation has its own reliability discipline. **Set the viewport explicitly** — defaults are much narrower than real monitors, and layouts that overflow at desktop width look perfectly centered at the default. **Wait on conditions**, not durations. **Load fonts and disable animation** before capturing anything.
