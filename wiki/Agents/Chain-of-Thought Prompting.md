...
---
**Chain-of-thought prompting** is a technique where a model is asked — or trained — to produce intermediate reasoning steps before stating a final answer, rather than jumping straight to a conclusion. The seminal result (Wei et al., Google, 2022) showed that simply adding "let's think step by step," or a few worked examples showing the reasoning explicitly, produced large accuracy gains on arithmetic and multi-step logic benchmarks compared to prompting for the answer directly — a surprising result at the time, since nothing about the model changed, only what it was asked to output.
