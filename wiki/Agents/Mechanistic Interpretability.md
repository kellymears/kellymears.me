...
---
**Mechanistic interpretability** is the attempt to reverse-engineer what a trained neural network actually computes, described as features and circuits, rather than treating it as a black box characterized only by its inputs and outputs. A **feature** is a direction in activation space corresponding to something describable; a **circuit** is a subgraph of features and weights implementing a behavior end to end. A published example is the induction head, which detects a repeated prefix and copies whatever followed it last time.
